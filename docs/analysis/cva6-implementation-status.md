...
| V-tables | Directed V1–V11 (branch-shadow, hazards, interrupts, reset/X…) | 🔵 | #27–#30, PR-5, N4 | G6, G7, G13 | test programs + `parser_wrap_tb.sv` + `tests/cva6-parser/{trap,parser_trap_v7}.S` | V1–V5/V7/V8/V9/V11 green (branch-shadow, interlock, RAW, WAW, adjacency, **faulting-squash V7 — N4**, redirect/exit, reset-X); V6 (N5) + V10 deferred (§3.1) |
...
| G7 interrupts/exceptions/ctx-switch | I5 (parse-exit) / V-tables / N4 (+ ctx-switch design) | 🔵 (V9 parse-**exit** redirect realized in I5 — on exit the FU steers fetch to a program-provided landing PC, exercised by all 22 cosim cases; **V7 faulting-instruction squash closed by N4** — an in-core `ecall` flushes an in-flight parser op, which re-executes and commits the fault-free result via the reusable `trap.S` scaffold; V6 interrupt-mid-parse (N5, reuses `trap.S`) + V10 context-switch remain deferred — §3.1) |
