...
| Escalation | riscv-dv + extended-Spike lock-step | 🔵 | T0 | G5, G6, G7, G11 | `nix/spike-tandem.nix` (source-built tandem Spike), `nix/cva6-parser-tandem.nix`, `scripts/cva6-parser-tandem.sh`, `scripts/cva6-baseline.sh` (`SPIKE_TANDEM` gate), `nix/cva6-parser/{tandem-get-misa-d-bit,tandem-mstatus-sd-mask}.patch` | **Stage 0 (base-ISA tandem) ✅**: `cva6-parser-tandem` stands up the dormant RVFI-vs-Spike lock-step — every retired RV64GC instruction of `base_isa.S` matched against a source-built extended Spike (insn/rd/pc/trap/mode + CSRs), **287/287, 0 mismatches**. Stages 1 (parser Spike extension → cosim under lock-step) + 2 (riscv-dv random) deferred |
