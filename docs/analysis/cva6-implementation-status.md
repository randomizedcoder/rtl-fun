...
| Escalation | riscv-dv + extended-Spike lock-step | ✅ | T0 | G5, G6, G7, G11 | `nix/spike-tandem.nix` + `nix/spike-tandem/parser_ext.cc` (source-built tandem Spike + parser extension), `model/libparsermodel/encoding.c` (`pm_decode`), `nix/spike-tandem/{parser_mmio,parser_shared}.h` (packet MMIO device), `nix/cva6-parser-tandem.nix`, `scripts/cva6-parser-tandem.sh` + `scripts/lib/{cosim,tandem}.sh`, `tests/cva6-parser/{base_isa,parser_tandem,cosim_main}.S`, `scripts/cva6-baseline.sh` (`SPIKE_TANDEM` gate), `nix/cva6-parser/{tandem-get-misa-d-bit,tandem-mstatus-sd-mask,tandem-parser-activate}.patch`; **Stage 2**: `verif/gen/gen_parser_rom.c` (`--random`/`--corpus` + `splitmix64`), `model/libparsermodel/pcap.c`, `scripts/cva6-parser-tandem-campaign.sh`, `nix/cva6-parser-tandem-campaign.nix` | **Stage 0 (base-ISA tandem) ✅**: `cva6-parser-tandem` stands up the dormant RVFI-vs-Spike lock-step — every retired RV64GC instruction of `base_isa.S` matched against a source-built extended Spike (insn/rd/pc/trap/mode + CSRs), **287/287, 0 mismatches**. **Stage 1a+1b (parser-op tandem) ✅**: a `pm_decode` inverse decoder (round-trip-proven) + a Spike `customext` extension reusing `libparsermodel` teach Spike the parser ISA; `parser_tandem.S` (custom-0 redirect + custom-3 register/CAM RD/WR/WRIMM) lock-steps — **43/0 mismatches**. **Stage 1c (packet cosim) ✅**: the tandem Spike also models the `0x5000_0000` MMIO packet peripheral (`parser_mmio.h`, a smart device sharing a mailbox with the extension), so the full 22-case `packet → flow_keys` corpus lock-steps — `PLOAD`/`PLENCUR` + the flow_keys/status readback all match, **22/22, exit codes -4/-13/-14/-15/-24**, deliberate-break negative controls confirmed live at each stage. **Stage 2 (random-packet campaign) ✅**: `cva6-parser-tandem-campaign` drives hundreds of seeded constrained-random + real xdp2-corpus packets through the same lock-step, **0 mismatches**, reproducible from a fixed committed seed (each case name self-repros). Two caveats: it randomizes the *packet* axis (full `riscv-dv` instruction interleaving stays deferred — commercial-UVM-only), and since both sides are `libparsermodel` the parser tandem proves RTL==model on random input |
