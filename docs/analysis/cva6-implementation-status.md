...
| G11 no negative control | N1 (neg. ctrl) / N6 (base-ISA) | 🔵 (negative control ✅ — `parser-negative-control` asserts the **stock** core traps the identical custom-0 word: illegal-instruction, mcause=2, handler → tohost=1 → fesvr SUCCESS, so the parser ops are a genuine ISA extension; base-ISA regression on the patched core is N6) |
