...
| `nix run .#cva6-parser-ctxsw-v10` | V10 (G7, ratifies **D7**): a **between-parse context switch**. The custom-3 move ABI spills thread A's parser register context to memory (`CPPRSRD`/`prs.mv.x.p`), a simulated thread B clobbers every writable p-reg, then A is reloaded (`CPPRSWR`/`prs.mv.p.x`) and asserted **bit-for-bit** — in-core over the real commit-gated pipeline. The round-trippable context is the read∩write subset `{p11,p13,p14,p15,p16}` |
| `nix run .#cva6-parser-ctxsw-mid` | M1 (G7, §3.1 item 4): a genuine **mid-parse context switch**. An async interrupt preempts a live parse; the ISR saves/clobbers/restores the resumable position+data registers — the ABI now also reaches the position state (`p1/p2/p6/p7/p8` writable, `p9` done read-only) — over the custom-3 move ABI (register-file only), and the parse **resumes to the golden model's byte-exact flow_keys**. Oracle = model final state + a `done==0` mid-parse marker; deterministic encodings in `parser_wrap_tb` Scenario 14. `done` is read-only (observed, not restored; mid-stream `done=1` wedges the frontend — deferred). Residual **M2** (meta_mem frame + CAM restore) stays deferred |
...
| In-core test programs | `tests/cva6-parser/` (`parser_insn.S`, `cosim_main.S`, `negctl.S`, `parser_trap_v7.S`, `parser_trap_v6.S`, `base_isa.S`, `trap.S`, `link.ld`) |
...
nix run .#cva6-parser-ctxsw-mid     # M1: a mid-parse context switch must save/restore the position+data registers and resume to the model flow_keys (G7, §3.1 item 4)
