...
def emit_clang_builtins_td(recs):
...
def emit_clang_builtins_inc(recs):
    """clang/include/clang/Basic/parser-clang-builtins.inc — two X-macro lists shared by the
    clang patch: PARSER_BUILTIN_LOWER (CodeGen -> InlineAsm) and PARSER_BUILTIN_RANGE (Sema
    immediate-range check). Define either (or both) before #include; the undefined one
    expands to nothing. Re-entrant (both macros are #undef'd at the end)."""
    out = [
        "/* GENERATED by tools/parser-gen from isa/parser-opcodes.yaml — do not edit. */",
        "/* Phase 7 C1: Clang __builtin_riscv_prs_* lowering + range tables (X-macros). */",
        "",
    ...
    out += ["", "#undef PARSER_BUILTIN_LOWER", "#undef PARSER_BUILTIN_RANGE", ""]
    return "\n".join(out)
