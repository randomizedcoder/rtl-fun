...
# --- Clang builtins (Phase 7 C1) ------------------------------------------------
# Each ASSEMBLY VARIANT of a custom-0 op becomes ONE __builtin_riscv_prs_* (the
# "mnemonic-form, per variant" surface): the size/attribute suffix cross-product (as
# emit_gas/emit_llvm) crossed with the destination discriminator (cam paccum/pnext). A
# builtin takes only the TRUE operands (the folded fields — Sz, the suffix fields, the
# dest — are fixed by the variant) as _Constant immediates, and lowers to an operand-less
# `asm volatile("<mnemonic> <dest?>, $0, ...")` with `i` operands: the immediates print as
# literals (no register, naked-safe like PRS_EMIT's `.insn 4, %0`) and the prs.* text is
# assembled by the parser-patched RISCV MC layer (the C0 leg). Presentation-only: the bits
# a builtin emits equal the model/binutils/llvm-mc word for the same op.


def _builtin_ops(insn):
    """The operands a builtin variant exposes: insn.operands minus the fields folded into
    the mnemonic (Sz via size_class, the `suffixes` fields) or the dest discriminator."""
    skip = set()
    if insn.size_class:
        skip.add("Sz")
    for grp in insn.suffixes:
        skip.add(grp["field"])
    if insn.dest and "field" in insn.dest:
        skip.add(insn.dest["field"])
    return [n for n in insn.operands if n not in skip]


def _builtin_variants(insn, sizes):
    """(dotted_suffix, match_delta, dest_keyword|None, is_discriminator) per builtin variant:
    the suffix cross-product (emit_gas' _suffix_variants) crossed with the dest. A cosmetic
    dest (load paccum / length pcurhdr) is a single fixed keyword; a discriminator dest (cam
    paccum=0 / pnext=1) expands to one variant per keyword, each pinning the field bit."""
    d = insn.dest
    if d and "field" in d and "map" in d:              # discriminator dest (cam D)
        hi, lo = insn.fields[d["field"]]
        dvars = [(kw, put(v, hi, lo), True) for kw, v in d["map"].items()]
    elif d and "token" in d:                            # cosmetic-fixed dest (load/length)
        dvars = [(d["token"], 0, False)]
    else:
        dvars = [(None, 0, False)]
    return [(sfx, sm | dm, kw, disc)
            for sfx, sm in _suffix_variants(insn, sizes)
            for kw, dm, disc in dvars]


def _builtin_records(insns, sizes):
    """Flatten the custom-0 ops into per-variant builtin records — the single source the
    three clang artifacts (.td decls, .inc lowering/range tables, .tsv test data) share.
    Custom-3 register moves are deferred to C3 (they need real GPR/p-reg operands)."""
    recs = []
    for i in insns:
        if i.opcode != C0_OPCODE:
            continue
        ops = _builtin_ops(i)
        for sfx, match_delta, dest_kw, disc in _builtin_variants(i, sizes):
            bid = (i.asm + sfx).replace(".", "_") + (f"_{dest_kw}" if disc else "")
            parts = ([dest_kw] if dest_kw else []) + [f"${k}" for k in range(len(ops))]
            asm = f"{i.asm}{sfx}" + (" " + ", ".join(parts) if parts else "")
            ranges = [(k, (1 << (i.fields[n][0] - i.fields[n][1] + 1)) - 1)
                      for k, n in enumerate(ops)]
            recs.append({"id": bid, "asm": asm, "nops": len(ops),
                         "match": i.match | match_delta, "ranges": ranges})
    return recs


# Representative p-registers the structural moves table (parser-clang-moves.tsv) exercises:
# paccum (15), pnext (11), pflags (16), and two numerics — spreads the Cpreg[28:24] pattern.
C3_TEST_PREGS = [15, 11, 16, 0, 3]


def _builtin_records_c3(insns):
    """Flatten the custom-3 register moves into builtin records (Phase 7 C3). Unlike the
    immediate-only C0 builtins, these carry real GPR operands: the p-register is a _Constant
    baked into the asm string as `p<N>`, the rd is an `=r` output (the builtin's return value),
    and the rs is an `r` input. Inline-asm $N is output-first. ld.immed (the immediate split-
    field form) is deferred — skipped here by its imm operands."""
    recs = []
    for i in insns:
        if i.opcode != C3_OPCODE:
            continue
        ops = _llvm_c3_ops(i)                     # [(ins, var, hi, lo)] in asm order
        vars_ = [o[1] for o in ops]
        if any(v.startswith("imm") for v in vars_):   # ld.immed — deferred
            continue
        has_out = "rd" in vars_
        idx, n, ninputs = {}, 0, 0
        if has_out:                                # output (rd) takes $0
            idx["rd"] = n
            n += 1
        for v in vars_:                            # then each input (rs1)
            if v == "rs1":
                idx["rs1"] = n
                n += 1
                ninputs += 1
        parts = ["{preg}" if var == "preg" else f"${idx[var]}"
                 for _ins, var, _hi, _lo in ops]
        asm = f"{i.asm} " + ", ".join(parts)
        constraints = (["=r"] if has_out else []) + ["r"] * ninputs
        # `uint64_t` (an exact type in clang's builtin-prototype grammar => "UWi") is the GPR
        # operand width on lp64d; bare "unsigned long" is qualifier-only there and fails tblgen.
        proto_args = ["_Constant unsigned int"] + ["uint64_t"] * ninputs
        proto = ("uint64_t" if has_out else "void") + "(" + ", ".join(proto_args) + ")"
        recs.append({"id": i.cname, "asm": asm, "constraints": ",".join(constraints),
                     "has_output": has_out, "ninputs": ninputs, "proto": proto,
                     "base_match": i.match})
    return recs
...
_CLANG_GEN_HEADER = [
    "// GENERATED by tools/parser-gen from isa/parser-opcodes.yaml — do not edit.",
    "// Phase 7 C1: Clang __builtin_riscv_prs_* (light path: builtin -> prs.* inline asm).",
    "",
]


def emit_clang_builtins_td(recs, c3recs=()):
    """clang/include/clang/Basic/BuiltinsRISCVXparser.td — one def per variant, included from
    BuiltinsRISCV.td next to the vendor XCV block. Empty Features => always available (no
    -march gate), mirroring the no-predicate MC design. Parser ops mutate state / emit an
    instruction, so they are NoThrow but NOT Const (Const would let the optimizer drop them).
    The C3 register moves (c3recs) append non-void protos with real GPR args (the p-register
    stays _Constant); they reuse the same class + NoThrow block."""
    out = list(_CLANG_GEN_HEADER)
    out += [
        'class RISCVXparserBuiltin<string prototype, string features = ""> : TargetBuiltin {',
    ...
    for r in recs:
        proto = "void(" + ", ".join(["_Constant unsigned int"] * r["nops"]) + ")"
        out.append(f'def {r["id"]} : RISCVXparserBuiltin<"{proto}">;')
    for r in c3recs:                              # C3 register moves (Phase 7 C3)
        out.append(f'def {r["id"]} : RISCVXparserBuiltin<"{r["proto"]}">;')
    out += ["} // Attributes = [NoThrow]", ""]
    return "\n".join(out)


def emit_clang_builtins_inc(recs, c3recs=()):
    """clang/include/clang/Basic/parser-clang-builtins.inc — X-macro lists shared by the clang
    patch: PARSER_BUILTIN_LOWER (C0 immediate CodeGen -> InlineAsm), PARSER_BUILTIN_LOWER_REG
    (C3 register-move CodeGen -> InlineAsm with =r/r constraints + a baked p-reg), and
    PARSER_BUILTIN_RANGE (Sema immediate-range check; also range-checks each C3 preg 0..31).
    Define any subset before #include; undefined ones expand to nothing. Re-entrant (all three
    macros are #undef'd at the end)."""
    out = [
        "/* GENERATED by tools/parser-gen from isa/parser-opcodes.yaml — do not edit. */",
        "/* Phase 7 C1/C3: Clang __builtin_riscv_prs_* lowering + range tables (X-macros). */",
        "",
        "#ifndef PARSER_BUILTIN_LOWER",
        "#define PARSER_BUILTIN_LOWER(ID, ASM, NOPS)",
        "#endif",
        "#ifndef PARSER_BUILTIN_LOWER_REG",
        "#define PARSER_BUILTIN_LOWER_REG(ID, ASM, CONS, HASOUT)",
        "#endif",
        "#ifndef PARSER_BUILTIN_RANGE",
        "#define PARSER_BUILTIN_RANGE(ID, ARG, MIN, MAX)",
        "#endif",
        "",
    ]
    for r in recs:
        out.append(f'PARSER_BUILTIN_LOWER({r["id"]}, "{r["asm"]}", {r["nops"]})')
    for r in c3recs:                             # C3 register moves (Phase 7 C3)
        hasout = 1 if r["has_output"] else 0
        out.append(f'PARSER_BUILTIN_LOWER_REG({r["id"]}, "{r["asm"]}", '
                   f'"{r["constraints"]}", {hasout})')
    out.append("")
    for r in recs:
        for arg, maxv in r["ranges"]:
            out.append(f'PARSER_BUILTIN_RANGE({r["id"]}, {arg}, 0, {maxv})')
    for r in c3recs:                             # the baked p-register selector, 0..31
        out.append(f'PARSER_BUILTIN_RANGE({r["id"]}, 0, 0, 31)')
    out += ["", "#undef PARSER_BUILTIN_LOWER", "#undef PARSER_BUILTIN_LOWER_REG",
            "#undef PARSER_BUILTIN_RANGE", ""]
    return "\n".join(out)


def emit_clang_builtins_data(recs):
    """toolchain/generated/parser-clang-builtins.tsv — drives parser-clang-builtins-test:
    calling __builtin_riscv_<id> with all-zero args emits exactly `match` (operand fields
    zero), so the test compiles a zero-arg call per builtin and checks word == match."""
    out = ["# GENERATED by tools/parser-gen — id<TAB>nops<TAB>match(hex)<TAB>asm-template",
           "# All-zero-args encoding == match (the fixed mnemonic/suffix/dest bits)."]
    for r in recs:
        out.append(f'{r["id"]}\t{r["nops"]}\t{r["match"]:08x}\t{r["asm"]}')
    return "\n".join(out) + "\n"


# Mask clearing the allocator-chosen Rs[19:15] + Rd[11:7] GPR fields; everything else is fixed.
_C3_STRUCT_MASK = 0xFFFFFFFF & ~((0x1F << 15) | (0x1F << 7))


def emit_clang_moves_data(c3recs):
    """toolchain/generated/parser-clang-moves.tsv — the C3 structural oracle (drives
    parser-clang-moves). Register moves have allocator-chosen rd/rs, so instead of a fixed word
    we pin the FIXED bits under a mask: one row per (move, representative p-register), where
    compiling a call with that preg must emit a word W with (W & mask) == fixed = base |
    (preg<<24). That checks the opcode/CoP/Func3/S/I/R bits AND that the selected p-register
    lands in Cpreg[28:24]. `ninputs`/`hasout` tell the runner how to shape the call."""
    out = ["# GENERATED by tools/parser-gen — id<TAB>preg<TAB>ninputs<TAB>hasout<TAB>"
           "fixed(hex)<TAB>mask(hex)",
           "# (emitted_word & mask) == fixed pins opcode/CoP/Func3/S/I/R + the p-register in",
           "# Cpreg[28:24]; mask clears the allocator-chosen Rs[19:15]/Rd[11:7]."]
    for r in c3recs:
        hasout = 1 if r["has_output"] else 0
        for preg in C3_TEST_PREGS:
            fixed = r["base_match"] | (preg << 24)
            out.append(f'{r["id"]}\t{preg}\t{r["ninputs"]}\t{hasout}\t'
                       f'{fixed:08x}\t{_C3_STRUCT_MASK:08x}')
    return "\n".join(out) + "\n"
...
def main():
    ...
    recs = _builtin_records(insns, sizes)
    c3recs = _builtin_records_c3(insns)
    with open(os.path.join(out_dir, "parser-clang-builtins.td"), "w") as f:
        f.write(emit_clang_builtins_td(recs, c3recs))
    with open(os.path.join(out_dir, "parser-clang-builtins.inc"), "w") as f:
        f.write(emit_clang_builtins_inc(recs, c3recs))
    with open(os.path.join(out_dir, "parser-clang-builtins.tsv"), "w") as f:
        f.write(emit_clang_builtins_data(recs))       # C0 only — byte-identical (C3 has no fixed word)
    with open(os.path.join(out_dir, "parser-clang-moves.tsv"), "w") as f:
        f.write(emit_clang_moves_data(c3recs))
    print(f"parser_gen: emitted {len(insns)} instructions "
          f"+ {len(recs)} clang builtins + {len(c3recs)} c3 move builtins to {out_dir}")
