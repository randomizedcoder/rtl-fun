...
int main(int argc, char **argv)
{
    ...
    if (strcmp(mode, "--corpus-blob") == 0) {
        /* ---- pack the whole pcap corpus into ONE C header for the in-core NIC ring
         * driver (tests/cva6-parser/nic_ring.c): per Ethernet packet, the frame bytes
         * (capped at the 256-byte MMIO packet window) + the golden model's flow_keys
         * and exit code. Unlike --corpus (one case dir per packet, for the per-ELF
         * suite runner), this is a single blob a single re-arming ELF loops over.
         *   --corpus-blob <PCAP_DIR> [MAX]   -> <out_dir>/corpus_blob.h
         * MAX=0/absent => all. We DON'T use the cases/ tree, so drop the manifest. */
        fclose(mf);
        remove(manifest);
        if (argc < 4) { fprintf(stderr, "usage: %s <dir> --corpus-blob <PCAP_DIR> [MAX]\n", argv[0]); return 1; }
        const char *pdir = argv[3];
        unsigned long maxc = (argc > 4) ? strtoul(argv[4], NULL, 0) : 0;   /* 0 = all */
        DIR *d = opendir(pdir);
        if (!d) { perror(pdir); return 1; }

        /* same deterministic collection+sort as --corpus, so the blob order (and thus
         * the gate) is stable regardless of readdir() order. */
        static char names[1024][256];
        int nn = 0;
        struct dirent *de;
        while ((de = readdir(d)) != NULL && nn < 1024) {
        ...
        FILE *hf = fopen(path(dir, "corpus_blob.h", buf, sizeof buf), "w");
        if (!hf) { perror("corpus_blob.h"); return 1; }
        fprintf(hf,
            "/* corpus_blob.h — GENERATED by gen_parser_rom --corpus-blob. Do not edit.\n"
            " * The whole pcap corpus packed as one image + the golden model's flow_keys\n"
            " * and exit code per packet, for the in-core NIC ring driver\n"
            " * (tests/cva6-parser/nic_ring.c). Packet bytes are capped at the 256-byte\n"
            " * MMIO packet window; corpus_len carries the true ParseLen (the parse bound,\n"
            " * matching the golden, which ran on the full frame). */\n"
            "#ifndef CORPUS_BLOB_H\n#define CORPUS_BLOB_H\n\n");

        /* buffers grown as packets are appended (two passes would need re-reading the
         * pcaps; instead accumulate into temp files, then splice). Simpler: emit the
         * per-packet arrays inline as we go via three string builders is overkill —
         * accumulate rows into memory. Cap: 1024 packets * 256 B is 256 KiB, fine. */
        static uint8_t  blob_pkt[1024][PKT_BUF_MAX];
        static uint16_t blob_len[1024];
        static int32_t  blob_code[1024];
        static uint8_t  blob_meta[1024][64];   /* sizeof(flow_keys) <= META window (64) */
        static char     blob_name[1024][300];
        size_t meta_len = 0;
        int emitted = 0, skipped = 0;
        for (int i = 0; i < nn; i++) {
            if (maxc && (unsigned long)emitted >= maxc) break;
            ...
            /* golden: run the model on the FULL frame (same as emit_case), so the blob's
             * expected flow_keys/code are byte-identical to the per-ELF suite goldens. */
            pstate ps;
            struct flow_keys fk;
            pm_init(&ps, p.b, p.n, &fk);
            int32_t code = pm_run(&ps, prog, n);   /* prog/n from pm_slice_program, top of main */
            meta_len = sizeof(fk);
            if (meta_len > 64) { fprintf(stderr, "gen: sizeof(flow_keys)=%zu > 64\n", meta_len); fclose(hf); return 1; }

            unsigned cap = (p.n < PKT_BUF_MAX) ? p.n : PKT_BUF_MAX;
            memcpy(blob_pkt[emitted], p.b, cap);          /* rest stays 0 */
            blob_len[emitted]  = (uint16_t)p.n;           /* true ParseLen */
            blob_code[emitted] = code;
            memcpy(blob_meta[emitted], &fk, meta_len);
            snprintf(blob_name[emitted], sizeof blob_name[0], "%.*s",
                     (int)(strlen(names[i]) - 5), names[i]);
            emitted++;
            fprintf(stderr, "  %-30s len=%-4u code=%-4d\n", blob_name[emitted-1], p.n, code);
        }
        if (emitted == 0) { fprintf(stderr, "gen: --corpus-blob found no Ethernet pcaps in %s\n", pdir); fclose(hf); return 1; }

        fprintf(hf, "#define CORPUS_N        %d\n", emitted);
        fprintf(hf, "#define CORPUS_PKT_MAX  %d\n", PKT_BUF_MAX);
        fprintf(hf, "#define CORPUS_META_LEN %zu\n\n", meta_len);

        fprintf(hf, "static const unsigned short corpus_len[CORPUS_N] = {\n");
        for (int i = 0; i < emitted; i++) fprintf(hf, "  %u,%s", blob_len[i], (i % 12 == 11) ? "\n" : "");
        fprintf(hf, "\n};\n\n");

        fprintf(hf, "static const int corpus_code[CORPUS_N] = {\n");
        for (int i = 0; i < emitted; i++) fprintf(hf, "  %d,%s", blob_code[i], (i % 12 == 11) ? "\n" : "");
        fprintf(hf, "\n};\n\n");

        fprintf(hf, "static const char *const corpus_name[CORPUS_N] = {\n");
        for (int i = 0; i < emitted; i++) fprintf(hf, "  \"%s\",\n", blob_name[i]);
        fprintf(hf, "};\n\n");

        fprintf(hf, "static const unsigned char corpus_pkt[CORPUS_N][CORPUS_PKT_MAX] = {\n");
        for (int i = 0; i < emitted; i++) {
            fprintf(hf, "  {");
            for (int b = 0; b < PKT_BUF_MAX; b++) fprintf(hf, "%u,", blob_pkt[i][b]);
            fprintf(hf, "},\n");
        }
        fprintf(hf, "};\n\n");

        fprintf(hf, "static const unsigned char corpus_meta[CORPUS_N][CORPUS_META_LEN] = {\n");
        for (int i = 0; i < emitted; i++) {
            fprintf(hf, "  {");
            for (size_t b = 0; b < meta_len; b++) fprintf(hf, "%u,", blob_meta[i][b]);
            fprintf(hf, "},\n");
        }
        fprintf(hf, "};\n\n#endif /* CORPUS_BLOB_H */\n");
        fclose(hf);
        fprintf(stderr, "gen: corpus-blob = %d packets emitted, %d skipped -> %s/corpus_blob.h%s\n",
                emitted, skipped, dir, maxc ? " [MAX cap applied]" : "");
        return 0;
    }

    fprintf(stderr, "gen: unknown mode '%s' (expected --suite | --random | --corpus | --corpus-blob)\n", mode);
    fclose(mf);
    return 1;
}
